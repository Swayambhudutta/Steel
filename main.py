...
def calculate_efficiency(Q_blast, Q_fuel, Q_air_comb, Q_flue, Q_shell):
    ...
    corrected_efficiency = raw_efficiency * 0.75  # Correction factor to bring into 70–80% range
...
def get_latest_cycle(df, stove_id):
    return df[df['stove_id'] == stove_id].sort_values('timestamp').iloc[-1]

def get_historical_data(df, stove_id):
    return df[df['stove_id'] == stove_id].sort_values('timestamp')

def get_condition_index(efficiency_list):
    if len(efficiency_list) == 0:
        return 0.0
    return np.mean(efficiency_list[-5:])  # Rolling mean of last 5 cycles
...
filtered_df = get_historical_data(df, stove_select)
...
cond_index = get_condition_index(efficiencies)
st.metric("Condition Index (Rolling Mean Efficiency)", f"{cond_index:.2f}%")
